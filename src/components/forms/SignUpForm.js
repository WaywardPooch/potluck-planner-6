...
function SignUpForm() {
  ...
  const [createdAccount, setCreatedAccount] = useState(false); //TEMP CODE, PLEASE DELETE
  ...
  const onSubmit = (event) => {
    ...
    schema.isValid(formData).then((valid) => {
      if (valid) {
        ...
        setCreatedAccount(true); //TEMP CODE, PLEASE DELETE
  ...
  const onChange = (event) => {
    ...
    if(name==='confirmPw'){
      if(formData.password===value){
        ...
        setCurrentError(formErrors.email ? formErrors.email : formErrors.name ? formErrors.name : formErrors.password);
      } else {
      ...
      .reach(schema, name)
      .validate(value)
      .then(() => {setFormErrors({ ...formErrors, [name]: "" }); setCurrentError(name!=='email'&&formErrors.email ? formErrors.email : name!=='name'&&formErrors.name ? formErrors.name : name!=='password'&&formErrors.password ? formErrors.password : formErrors.confirmPw);})
  ...
  return (
    <form onSubmit={onSubmit}>
      {createdAccount && <p>You have created an account</p>} {/*TEMP CODE, PLEASE DELETE*/}
